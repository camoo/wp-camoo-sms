...
class Subscribers_Groups_List_Table extends \WP_List_Table
{
    ...
    public function process_bulk_action()
    {
        ...
        // Search action
        if (isset($_GET['s'])) {
            $prepare     = $this->db->prepare("SELECT * from `{$this->tb_prefix}camoo_sms_subscribes_group` WHERE name LIKE %s", '%' . $this->db->esc_like(sanitize_text_field($_GET['s'])) . '%');
        ...
        if ('bulk_delete' === $this->current_action()) {
            foreach ($_GET['id'] as $id) {
                $this->db->delete($this->tb_prefix . "camoo_sms_subscribes_group", array( 'ID' => sanitize_key($id) ));
            }
        ...
        if ('delete' === $this->current_action()) {
            $this->db->delete($this->tb_prefix . "camoo_sms_subscribes_group", array( 'ID' => sanitize_key($_GET['ID']) ));
            $this->data  = $this->get_data();
            $this->count = $this->get_total();
    ...
    public function prepare_items()
    {
        ...
        usort($data, '\CAMOO_SMS\Subscribers_Groups_List_Table::usort_reorder');
    ...
    public function get_data($query = '')
    {
        ...
        if (! $query) {
            $query = 'SELECT * FROM `' . $this->tb_prefix . 'camoo_sms_subscribes_group` LIMIT ' . $this->limit . ' OFFSET ' . $page_number;
        } else {
    ...
    {
        if (! $query) {
            $query = 'SELECT * FROM `' . $this->tb_prefix . 'camoo_sms_subscribes_group`';
        }
        $result = $this->db->get_results($query, ARRAY_A);
        $result = count($result);
